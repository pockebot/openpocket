...
function hasTool(availableToolNames: string[] | undefined, name: string): boolean {
  if (!Array.isArray(availableToolNames) || availableToolNames.length === 0) {
    return true;
  }
  return availableToolNames.includes(name);
}
...
function buildSkillsSection(params: {
  ...
  const trimmedSkills = params.skillsSummary.trim();
  const trimmedPreloaded = params.activeSkillsText?.trim() || "";
  const canReadSkills = hasTool(params.availableToolNames, "read");
  const hasSkillsIndex = canReadSkills
    && trimmedSkills.length > 0
    && trimmedSkills !== "(no skills loaded)";

  if (!hasSkillsIndex && !trimmedPreloaded) {
    return [];
  }
...
export function buildSystemPrompt(
  ...
  if (mode === "none") {
    return [
      ...
      "Prefer runtime_info first for authoritative runtime metadata before scanning logs/files.",
      "For Android permission dialogs, handle locally: tap Allow if the permission is relevant to the current task, tap Deny if not. Never call request_human_auth for OS permission dialogs.",
  ...
  if (mode === "minimal") {
    return [
      ...
      "",
      ...buildSkillsSection({
        skillsSummary: trimmedSkills,
        availableToolNames: options?.availableToolNames,
        activeSkillsText: options?.activeSkillsText,
      }),
  ...
  return [
    ...
    "- Android permission dialogs (notifications/photos/files/camera/microphone/location/network/etc.) must be handled locally by the agent.",
    "- Evaluate task relevance: tap Allow if the permission is needed for the current task; tap Deny/Don't Allow if the permission is irrelevant to the task.",
    "- Do not call request_human_auth for Android permission dialogs. request_human_auth is for Human Phone data/authorization only.",
...
export function buildUserPrompt(
  ...
  return [
    ...
    "6) For Android permission dialogs, evaluate task relevance: tap Allow if the permission is needed, tap Deny if irrelevant. Never call request_human_auth for OS permission dialogs.",
