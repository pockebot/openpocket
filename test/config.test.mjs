...
test("loadConfig migrates legacy snake_case return_home_on_task_end", async () => {
  await withTempHome("openpocket-config-migrate-", async (home) => {
    ...
    assert.equal(cfg.target.virtualPhonePin, "2468");
    assert.equal(cfg.target.physicalPhonePin, "1357");
    ...
    assert.equal(saved.target.virtualPhonePin, "2468");
    assert.equal(saved.target.physicalPhonePin, "1357");
