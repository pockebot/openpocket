...
test("ChatAssistant decide detects implicit Chinese schedule intent and prepares confirmation preview", async () => {
  ...
  assistant.extractScheduleIntentWithModel = async () => ({
    intent: {
      ...
      confirmationPrompt: "I understand this as a scheduled job: Every day at 08:00 (Asia/Shanghai), task \"Open Slack and complete check-in\". Reply \"confirm\" to create it or \"cancel\" to discard it.",
    },
    confidence: 0.98,
...
test("ChatAssistant decide treats one-shot tomorrow phrasing as schedule intent", async () => {
  ...
  assistant.extractScheduleIntentWithModel = async () => ({
    intent: {
      ...
      delivery: null,
      requiresConfirmation: true,
      confirmationPrompt: "I understand this as a scheduled job: Tomorrow at 08:00 (Asia/Shanghai), task \"Open Slack and complete check-in\". Reply \"confirm\" to create it or \"cancel\" to discard it.",
    },
    confidence: 0.98,
    reason: "schedule_model",
  });
